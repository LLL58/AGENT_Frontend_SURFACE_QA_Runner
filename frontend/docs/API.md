...
```typescript
import { RouteRunner, routes } from 'surface-qa-agent-runner';
...
| saveArtifacts | Page, string | Promise<{screenshotPath, htmlPath, domSnapshotPath}> | 保存证据 |
...
console.log(snapshot.pageErrors);
console.log(snapshot.networkErrors);
```
...
| 方法 | 参数 | 返回值 | 说明 |
|------|------|--------|------|
| scan | Page, number | Promise<ControlCandidate[]> | 扫描控件 |
